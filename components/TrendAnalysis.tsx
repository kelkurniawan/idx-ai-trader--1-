...
const TrendPill: React.FC<{ label: string; trend: TrendDirection }> = ({ label, trend }) => {
  ...
  return (
    <div className={`flex items-center justify-between p-3 md:p-3.5 rounded-xl border ${config.bg} ${config.border} transition-all hover:scale-[1.01]`}>
      <span className={`text-xs md:text-sm font-bold ${config.text}`}>{label}</span>
      <div className="flex items-center gap-1.5 md:gap-2">
        <span className={`text-[9px] md:text-[10px] font-black tracking-wider px-2 md:px-2.5 py-0.5 md:py-1 rounded-lg ${config.badge}`}>{trend}</span>
...
const TrendAnalysis: React.FC<TrendAnalysisProps> = ({
  ...
  return (
    <div className="glass-card rounded-2xl p-5 md:p-6 relative overflow-hidden animate-slide-up h-full">
      ...
      <h3 className="text-slate-800 dark:text-slate-100 text-xs md:text-sm font-black uppercase tracking-widest mb-4 md:mb-5 flex items-center gap-2 md:gap-2.5">
        <div className="w-7 h-7 md:w-8 md:h-8 rounded-lg bg-blue-50 dark:bg-blue-500/10 flex items-center justify-center">
          <svg className="w-3.5 h-3.5 md:w-4 md:h-4 text-blue-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <div className="mt-4 md:mt-5 pt-4 md:pt-5 border-t border-slate-200/60 dark:border-slate-700/40 grid grid-cols-2 gap-3 md:gap-4">
        <div>
          <div className="text-[9px] md:text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-0.5 md:mb-1">vs MA50</div>
          <div className={`font-mono text-base md:text-lg font-black ${diffMa50 >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-600 dark:text-red-400'}`}>
            {diffMa50 >= 0 ? '+' : ''}{diffMa50.toFixed(2)}%
          </div>
          {/* Visual bar */}
          <div className="mt-1 md:mt-1.5 h-1 rounded-full bg-slate-100 dark:bg-slate-800 overflow-hidden">
            <div
        ...
        <div className="text-right">
          <div className="text-[9px] md:text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-0.5 md:mb-1">vs MA200</div>
          <div className={`font-mono text-base md:text-lg font-black ${diffMa200 >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-600 dark:text-red-400'}`}>
